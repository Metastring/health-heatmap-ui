...
const hiddenLayers = [{ id: "IBP-Layer-4" }, { id: "IBP-Layer-5" }];
...
function IBPLayers() {
  ...
  useEffect(() => {
    const shownLayers = hiddenLayers.length
      ...
      : layers;
    setFilteredLayers(
      q ? filterLayers(JSON.parse(JSON.stringify(shownLayers)), q) : shownLayers
    );
  }, [q, layers]);
  ...
  const removeThisLayerFromLoadingStack = (layer) => {
    setLayersLoading(
      JSON.parse(
        JSON.stringify(_.filter(layersLoading, (l) => layer.id !== l.id))
      )
    );
  };
